...
//Attack Stats
	...
	private void Update()
	{
		...
		// Handle looping shoot sound for some kindOfTurret
		switch (kindOfTurret)
		{
			case KindOfTurret.Spliter:
			case KindOfTurret.Furnace:
	...
	public IEnumerator Shoot(EnemiesTemp enemy)
	{
		...
		// Instantiate shaders and particles 
		switch (kindOfTurret)
		{
			...
			case KindOfTurret.Immobilizer:
				{
					...
					PlayerPrefs.SetInt("ZapAtk", PlayerPrefs.GetInt("ZapAtk", 0) + 1);
					if (PlayerPrefs.GetInt("ZapAtk") >= 2000) AchivementsFinishing.instance.Achievement(true, GPGSIds.achievement_fast_has_litghning);
		...
		if (delayBeforeShoot == 0 && kindOfTurret != KindOfTurret.Zap)
		{
						...
						if (isInsideCone)
						{
							...
							if (target.nbrOfAtqSuffed >= capPassive/*5*/)
							{
								...
								if (!target.isBurning)
								{
									...
									PlayerPrefs.SetInt("NbBruning", PlayerPrefs.GetInt("NbBruning", 0) + 1);
									if (PlayerPrefs.GetInt("NbBruning") >= 4000) AchivementsFinishing.instance.Achievement(true, GPGSIds.achievement_hot_hot_hot);
								}
							}
						}
			...
			case KindOfTurret.Discord:
				{
					...
					PlayerPrefs.SetInt("StackDamage", PlayerPrefs.GetInt("StackDamage", 0) + 5 * atqBonusStack);
					if (PlayerPrefs.GetInt("StackDamage") >= 200000) AchivementsFinishing.instance.Achievement(true, GPGSIds.achievement_its_over_200k);
			...
			case KindOfTurret.Immobilizer:
				{
					...
					PlayerPrefs.SetFloat("Freeze", PlayerPrefs.GetFloat("Freeze", 0f) + 0.5f);
					if (PlayerPrefs.GetFloat("Freeze") == 1800) AchivementsFinishing.instance.Achievement(true, GPGSIds.achievement_freeze);

					break;
				}
			case KindOfTurret.Zap:
				{
					StartCoroutine(ZapStackAttackSpeed());
					break;
				}
			case KindOfTurret.Generator:
				{
					int gain = (int)(atqPoints * basePassiveParameters /*50*/ / 100);
					gameManager.truck.gold += gain;

					PlayerPrefs.SetInt("RichGold", PlayerPrefs.GetInt("RichGold", 0) + gain);
					if (PlayerPrefs.GetInt("RichGold") >= 9000) AchivementsFinishing.instance.Achievement(true, GPGSIds.achievement_rich_has_cresus);
			...
			case KindOfTurret.Zap: 
				{
					if (speedStack >= 7)
					{
						foreach (EnemiesTemp enemies in inRangeEnemies)
						{
							enemies.TakeDamage(atqPoints * 20);
						}

						StartCoroutine(Stase());
					}
	...
	public void Upgrade()
	{
		...
		switch (currentLevel)
		{
			...
			case 2:
                ...
                {
					case KindOfTurret.Basic:
						PlayerPrefs.SetInt("BasicMax", 1);
						break;
					case KindOfTurret.Mortar:
						PlayerPrefs.SetInt("MortarMax", 1);
						break;
					case KindOfTurret.Discord:
						PlayerPrefs.SetInt("DiscordMax", 1);
						break;
					case KindOfTurret.SniperTower:
						PlayerPrefs.SetInt("SniperMax", 1);
						break;
					case KindOfTurret.Furnace:
						PlayerPrefs.SetInt("FurnaceMax", 1);
						break;
					case KindOfTurret.Channelizer:
						PlayerPrefs.SetInt("ChanneMax", 1);
						break;
					case KindOfTurret.Generator:
						PlayerPrefs.SetInt("GenerMax", 1);
						break;
					case KindOfTurret.Immobilizer:
						PlayerPrefs.SetInt("ImmoMax", 1);
						break;
					case KindOfTurret.Zap:
						PlayerPrefs.SetInt("ZapMax", 1);
						break;
					case KindOfTurret.Spliter:
						PlayerPrefs.SetInt("SplitterMax", 1);
						break;
				}
				if(PlayerPrefs.GetInt("BasicMax", 0) == 1 && PlayerPrefs.GetInt("MortarMax", 0) == 1 && PlayerPrefs.GetInt("DiscordMax", 0) == 1 &&
				   PlayerPrefs.GetInt("SniperMax", 0) == 1 && PlayerPrefs.GetInt("FurnaceMax", 0) == 1 && PlayerPrefs.GetInt("ChanneMax", 0) == 1 && 
				   PlayerPrefs.GetInt("GenerMax", 0) == 1 && PlayerPrefs.GetInt("ImmoMax", 0) == 1 && PlayerPrefs.GetInt("ZapMax", 0) == 1 && 
				   PlayerPrefs.GetInt("SplitterMax", 0) == 1)
                {
					AchivementsFinishing.instance.Achievement(true, GPGSIds.achievement_overpowered);
                }
	...
	private IEnumerator ZapStackAttackSpeed()
	{
		if (speedStack < 7)
		{
			speedStack++;
	...
	private IEnumerator Stase()
	{
		spriteRenderer.color = Color.gray;

		bullet = Instantiate(particleShoot, particleSpawnPoint.transform.position, Quaternion.Euler(-rotZ, 90, 180), this.transform.GetChild(0));
		bullet.GetComponent<ParticleSystem>().startSize = 10;
		bullet.gameObject.transform.GetChild(2).GetComponent<ParticleSystem>().startSize = 10;
